...
Section~\ref{sec:ch4-budget} measured allocation headroom with one retriever on
one panel. This section tests whether that result depends on those choices.
A complementary BM25 study applies the passage-slot accounting to MuSiQue,
2WikiMultiHopQA~\cite{ho2020constructing} and HotpotQA~\cite{yang2018hotpotqa}. It uses
...
Each source panel contains $1\,000$ questions. Trace generation was run to the
compute budget available for this study, retaining $824$ MuSiQue, $977$ 2Wiki
and $996$ HotpotQA questions. All comparisons, including the full-question
control, use those retained IDs, so coverage is conditional on obtaining a
trace rather than a success rate over the full intake. Retention follows
execution order and per-question cost, not any retrieval or answer outcome. The source
...
policy class. The headroom measured in Section~\ref{sec:ch4-budget} therefore
appears under a sparse retriever and on all three benchmarks, rather than only
on the dense-retrieval MuSiQue panel.
...
reaches coverage $50\%$, $60\%$ and $70\%$ at $16$, $32$ and $128$ slots.
...
Generated plan, paraphrased references & $824$ & $10.9$ & $39.7$ \\
\hline
\end{tabular}
\end{table}
...
the sequential fixed-list oracle covers $79.5\%$ of the retained MuSiQue
questions, compared with balanced coverage $73.3\%$. Both results concern
